...
export default async function({user, targetEvent, targetUsers, roleType}) {
	// Check permissions
	...
	const newRoles = []
	for(const targetUser of targetUsers) {
		// Invite participant to event (in case not already a participant)
		await targetEvent.inviteParticipant(targetUser, undefined, false)
		
		const newRole = await targetEvent.setRole(targetUser, roleType, false)
